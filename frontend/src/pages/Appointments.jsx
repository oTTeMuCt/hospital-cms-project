    ...
    <div>
      <div className="page-header">
        <div className="flex-between">
          <div><h1>Записи на приём</h1><p>Управление расписанием и записями пациентов</p></div>
      ...
      <div className="page-content">
        ...
        {appointments.length === 0 ? (
          <div className="empty-state">
            ...
            <h3>Нет записей</h3><p>Создайте первую запись на приём</p>
            <button className="btn btn-primary" onClick={() => setShowModal(true)}>+ Новая запись</button>
          </div>
        ) : (
          <div className="table-wrap">
            <table>
              <thead><tr><th>Пациент</th><th>Врач</th><th>Дата и время</th><th>Статус</th><th>Причина</th><th style={{ width: "160px" }}>Действия</th></tr></thead>
              <tbody>
                {appointments.map((a) => {
                  const st = STATUS_LABELS[a.status] || { label: a.status, cls: "" };
                  return (
                    <tr key={a.id}>
                      <td style={{ fontWeight: 600 }}>{a.patient_name || a.patient?.full_name || a.patient?.user?.full_name_display || `#${a.patient}`}</td>
                      <td>{a.doctor_name || a.doctor?.full_name || a.doctor?.user?.full_name_display || `#${a.doctor}`}</td>
                      ...
                      <td>
                        <div style={{ display: "flex", gap: "6px" }}>
                          {a.status === "pending" && <button className="btn btn-success btn-sm" onClick={() => updateStatus(a.id, "confirmed")}>✅</button>}
                          {a.status === "confirmed" && <button className="btn btn-info btn-sm" onClick={() => updateStatus(a.id, "completed")}>✔️</button>}
                          {(a.status === "pending" || a.status === "confirmed") && <button className="btn btn-danger btn-sm" onClick={() => updateStatus(a.id, "cancelled")}>❌</button>}
                        </div>
                      </td>
                    </tr>
                  );
        ...
        {showModal && (
          <div className="modal-overlay" onClick={() => setShowModal(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header"><h2>Новая запись на приём</h2><button className="close-btn" onClick={() => setShowModal(false)}>×</button></div>

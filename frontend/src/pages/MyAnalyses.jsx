...
export default function MyAnalyses() {
  ...
  useEffect(() => {
    const fetchMyAnalyses = async () => {
      try {
        // Получаем профиль пациента текущего пользователя
      ...
      <div className="page-content">
        ...
        {analyses.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="flex-center flex-gap mt-4">
              <a
                href="https://t.me/HospitalCMSbot"
                target="_blank"
                rel="noopener noreferrer"
                className="badge badge-info"
                style={{ cursor: "pointer", textDecoration: "none" }}
              >
                💬 Telegram: @HospitalCMSbot
              </a>
              ...
              <tbody>
                {analyses.map((a, idx) => {
                  ...
                  return (
                    <tr key={a.id}>
                      <td>{idx + 1}</td>
                      <td style={{ fontWeight: 700 }}>{a.analysis_type_name || a.analysis_type?.name || `#${a.analysis_type}`}</td>

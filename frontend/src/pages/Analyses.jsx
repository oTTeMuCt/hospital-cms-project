...
export default function Analyses() {
  ...
  const handleSaveResult = async (e) => {
    e.preventDefault(); if (!resultModal) return;
    try { await api.patch(`/analysis-orders/${resultModal}/`, { ...resultForm, status: "completed" }); setResultModal(null); fetchData(); }
    catch (err) { setError(err.response?.data ? JSON.stringify(err.response.data) : "Ошибка сохранения результата"); }
  };
      ...
      <div className="page-content">
                  ...
                  <tr key={o.id}>
                    ...
                    <td>
                      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                        ...
                        {o.status === "in_progress" && <button className="btn btn-success btn-sm" onClick={() => { setResultModal(o.id); setResultForm({ result: o.result || "", notes: o.notes || "" }); }}>✅ Результат</button>}
        ...
        {resultModal && (
          <div className="modal-overlay" onClick={() => setResultModal(null)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header"><h2>Загрузить результат</h2><button className="close-btn" onClick={() => setResultModal(null)}>×</button></div>
              <form onSubmit={handleSaveResult}><div className="modal-body">
                <div className="form-group"><label>Результат анализа *</label><textarea className="input" value={resultForm.result} onChange={(e) => setResultForm({ ...resultForm, result: e.target.value })} required rows={4} placeholder="Введите результат анализа..." /></div>
                <div className="form-group"><label>Примечания</label><textarea className="input" value={resultForm.notes} onChange={(e) => setResultForm({ ...resultForm, notes: e.target.value })} rows={2} /></div>
              </div>
              <div className="modal-footer"><button type="button" className="btn btn-outline" onClick={() => setResultModal(null)}>Отмена</button><button type="submit" className="btn btn-success">Сохранить результат</button></div></form>
            </div>
          </div>
        )}
      </div>
